...
#define FREQ_MEM_SIZE sizeof(float) // float = 32 bits, 4 bytes => 4 (Yo)
#define ATT_MEM_SIZE sizeof(uint8_t) // uint8_t = 8 bits, 1 byte => 1 (Yo)
...
void ADF4350_synthetizer(float fref, float freq, uint8_t power, uint32_t LE) // 16, 4200
{
	...
	uint8_t twoexp = pow(2, (uint8_t)(log(4400.0 / freq) / log(2))); // Dos elevado a log en base 2 de 4400/freq (Yo)
	...
	uint8_t DIVIDER_SELECT = log(twoexp) / log(2); // logaritmo en base 2 de twoexp	(Yo)			// 0 = /1 ; 1 = /2 ; 2 = /4 ; 3 = /8 ; 4 = /16
	...
	SPI_transfer(0x00); // RESERVADO
	SPI_transfer(0x05); // Register 5 - Control bits (Yo)
	delay16b(ADFDEL);   // 10 ns minimum (Yo)
	DPIN->PIO_SODR |= LE;
	
	// Configure clock divider REGISTER 4 (Yo)
	DPIN->PIO_CODR |= LE;
	delay16b(ADFDEL);
	SPI_transfer(0); // RESERVADO
	SPI_transfer(DIVIDER_SELECT << 4 | 143); // DIVIDER SELECT & MSB CLOCK DIVIDER VALUE: (1 1 1 1-> DB19 DB18 DB17 DB16) Clock divider: 255 (Yo)
	...
	SPI_transfer(0xF0); // Select LSB clock divider value: (1 1 1 1 -> DB15 DB14 DB13 DB12) Clock divider: 255 (Yo)
	...
	SPI_transfer(power << 3 | 36); // Power: +5 (1 1) (Yo)
	...
	// Configure Clock divider REGISTER 3 (Yo)
	DPIN->PIO_CODR |= LE;
	delay16b(ADFDEL);
	SPI_transfer(0x00); // RESERVADO
	SPI_transfer(0x00); // Clock divider off (Yo) - bit mas significativo del modo divisor de reloj: 0b0. Resultado total. 0b00. Equivale a divisor de reloj desactivado
						// Cycle slip reduction desactivado
	SPI_transfer(0x04); // 7 MSB clock divider value: 0 0 0 0 1 0 0 -> Clock divider: 150 (Yo) - los 7 bits mas significantes del valor de divisor de reloj de 12-bit: 0b0000100. Resultado total: 0b000010010110 0x096 150
						// bit menos significativo del modo divisor de reloj: 0b0. Resultado total. 0b00. Equivale a divisor de reloj desactivado
	SPI_transfer(0xB3); // 5 LSB clock divider value: 1 0 1 1 0 -> Clock divider: 150 (Yo)
	...
	DPIN->PIO_CODR |= LE;
	delay16b(ADFDEL);
	SPI_transfer(0x00); // Low noise mode (Yo)
						// Modo de bajo ruido (y no minimizando espureas)
						// Salida tri estado
						// Duplicador de la frecuencia referencia desactivado
	SPI_transfer(0x00); // 8 bits mas significativos del contador R: 0x00. Resultado total: 0b0000000001. Equivale a divisor R = 1
	SPI_transfer(0x4E); // LDF FRAC-N, 2.5mA  ajuste de corriente de la bomba de carga, doble buffer desactivado, divisor R = 1 (Yo)
	...
	SPI_transfer(0x42); // Counter reset and power-down disable, CP tri-estadp desactivada, counter reset disable, PD polarity positive, LDP = 10 ns (Yo)
	...
	DPIN->PIO_CODR |= LE;
	delay16b(ADFDEL);
	SPI_transfer(0x08);	// Reservado
						// Prescaler 8/9
						// 3 bits mas significativos del valor de fase en 12 bit: 0b000. Resultado total: 0x001	(Es el que recomienda el datasheet)
	SPI_transfer(0x00);	// 8 bits siguientes del valor de fase en 12 bit: 0x00.	Resultado total: 0x001	(Es el que recomienda el datasheet)
	SPI_transfer((MOD & 0xFE0) >> 5 | 128); // Bit menos significativo restante del valor de fase en 12 bit: 0b1. Resultado total: 0x001 (Es el que recomienda el datasheet).
	...
	SPI_transfer((MOD & 0x1F) << 3 | 1);    // Si se tiene un modulo en 12 bits (con valor max 4096, por lo tanto), se le cogen sus ultimos 5 bits y junto con un 0b001 como bits de control se completa el byte
	...
	DPIN->PIO_CODR |= LE;
	delay16b(ADFDEL);
	SPI_transfer((INT & 0xFE00) >> 9); // Del valor entero de 16 bit, se le cogen los primeros 7 bits y se le añade un 0b0 reservado a la izquierda
	SPI_transfer((INT & 0x1FE) >> 1);  // Del valor entero de 16 bit, se le cogen los bits del 8 al 15
	SPI_transfer((INT & 1) << 7 | (FRAC & 0xFE0) >> 5); // Del valor entero de 16 bits, se le coge el ultimo. A eso se añaden los 7 primeros bits del valor de la fraccion de 12 bit
	SPI_transfer((FRAC & 0x1F) << 3);  // Del valor de la fraccion de 16 bit se le cogen los 5 ultimos bits del valor de la fraccion de 12 bit y el 0b000 de los bits de control correspondientes a este registro
	delay16b(ADFDEL);
	DPIN->PIO_SODR |= LE;
}
...
void AD9914_sweepInit(float fclk, float fstp)
{
	...
	SPI_transfer(0x01); // [31:24] -> Calibracion del DAC activada
	...
	SPI_transfer(0x03); // Direccion CFR4 
	SPI_transfer(0x00); // [31:24] -> Calibracion del DAC desactivada
	SPI_transfer(0x05); // [23:16] -> Default value
	SPI_transfer(0x21); // [15:8]  -> Default value
	SPI_transfer(0x20); // [7:0]   -> Default value
	DPIN->PIO_SODR |= IOUPD;
	...
	SPI_transfer(0x00); // Direccion CFR1
	SPI_transfer(0x00); // [31:24] 
	SPI_transfer(0x01); // [23:16] -> Salida senosoidad habilitada (no cosenoidal)
	SPI_transfer(0x00); // [15:8]  -> No autoclear of DRG and no OSK (Yo)
	SPI_transfer(0x08); // [7:0]   -> Enable external powerdown control (Yo) - // EXT_PWR_DOWN apaga completamente todos los bloques y reinciarlo requeriria calibracion
	DPIN->PIO_SODR |= IOUPD;
	DPIN->PIO_CODR |= IOUPD;
	
	// CRF2 dwelling configuration
	SPI_transfer(0x01); // Direccion CFR2
	SPI_transfer(0x00); // [31:24] -> Open
	SPI_transfer(0x0E); // [23:16] -> active no-dwell low and no-dwell high (0x0E -> 0000 1110) - DRG activo, No dwell alto y bajo activos  
	SPI_transfer(0x00); // [15:8]  -> DROVER disabled + SYNC_CLK disabled (DROVER esta no conectado)
	SPI_transfer(0x00); // [7:0]   -> Open
	DPIN->PIO_SODR |= IOUPD;
	DPIN->PIO_CODR |= IOUPD;
	
	// CRF3 - PLL Configuration (default values)
	// No se usa el PLL
	SPI_transfer(0x02); // Direccion CFR3
	SPI_transfer(0x00); // [31:24]
	SPI_transfer(0x00); // [23:16]
	SPI_transfer(0x19); // [15:8]
	SPI_transfer(0x1C); // [7:0]
	DPIN->PIO_SODR |= IOUPD;
	DPIN->PIO_CODR |= IOUPD;
	...
	SPI_transfer((parametroDwelling & 0x000000FF));      // 0x01 // Se usa el minimo negative slope rate no nulo posible (P = 1 de los posibles en el margen 0 <-> 2^16-1)
	...
	SPI_transfer((parametroDwelling & 0x0000FF00) >> 8); // 0x00
	SPI_transfer((parametroDwelling & 0x000000FF));      // 0x01 // Se usa el minimo positive slope rate no nulo posible (valor 1 de los posibles en el margen 0 <-> 2^16-1)
...
void AD9914_Sweep(float fclk, float fstart, float fstop, uint8_t att)
{
	...
	SPI_transfer(0x01); // Direccion CFR2
	SPI_transfer(0x00); // [31:24] -> Open
	SPI_transfer(0x00); // [23:16] -> Digital ramp disabled + No dwell high disabled + No dwell low disabled
	SPI_transfer(0x00); // [15:8]  -> DROVER disabled + SYNC_CLK disabled (DROVER esta no conectado)
	SPI_transfer(0x00); // [7:0]   -> Open
	DPIN->PIO_SODR |= IOUPD;
	DPIN->PIO_CODR |= IOUPD;
	...
	// Rising Digital Ramp Step Size Register
	//Esta al inicio de la ventana y va al final en pasos de FTWstep (Yo)
	SPI_transfer(0x06);
	SPI_transfer((FTWstep & 0xFF000000) >> 24); // [31:24]
	SPI_transfer((FTWstep & 0x00FF0000) >> 16); // [23:16]
	SPI_transfer((FTWstep & 0x0000FF00) >> 8);  // [15:8]
	SPI_transfer((FTWstep & 0x000000FF));       // [7:0]
	DPIN->PIO_SODR |= IOUPD;
	// Habria que esperar a que el DROVER estuviera a 1 y esperar 2 ciclos de reloj que es lo que dura (end pag. 27 datasheet AD9914) (Yo)
	...
	else // Esta al final de la ventana y vuelve en pasos de FTWstep hacia el inicio de la ventana (Yo)
	{
		//Falling Digital Ramp Step Size Register
		SPI_transfer(0x07);
		SPI_transfer(((FTWstep) & 0xFF000000) >> 24); // [31:24]
	...
	SPI_transfer(0x01); // Direccion CFR2
	SPI_transfer(0x00); // [31:24] -> Open
	SPI_transfer(0x0E); // [23:16] -> Digital ramp enabled + No dwell high enabled + No dwell low enabled
	SPI_transfer(0x00); // [15:8]  -> DROVER disabled + SYNC_CLK disabled (DROVER esta no conectado)
	SPI_transfer(0x00); // [7:0]   -> Open
	DPIN->PIO_SODR |= IOUPD;
	DPIN->PIO_CODR |= IOUPD;
...
#if !defined(MALETA_1) && !defined(ENRACKABLE_1) &&	!defined(MALETA_2) && !defined(ENRACKABLE_2) &&	!defined(MALETA_3) && !defined(ENRACKABLE_3)
	DPIN->PIO_OER |= CSLO;
	DPIN->PIO_SODR |= CSLO;
#endif
	
#if defined(MALETA_5A) || defined(MALETA_5B) || defined(ENRACKABLE_5A) || defined(ENRACKABLE_5B)
