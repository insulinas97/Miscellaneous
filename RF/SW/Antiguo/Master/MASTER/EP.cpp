...
/*
*	Enable Etapa de potencia
*	========================
*	-Activacion:
*		1-. Cuando se recibe un comando 0 (Encender EP) por la UART
*	-Desactivacion:
*		1-. En el arranque del programa del uC master
*		2-. Cuando se recibe un comando 1 (Apagar EP) por la UART
*
*	Enable DC-DC
*	============
*	-Activacion:
*		1-. Cuando se recibe un comando 2 (Configurar parametros inhibidor)
*	-Desactivacion:
*		1-. En el arranque del programa del uC master
*		2-. En alarmSet(), cuando se ha superado algun umbral: Temp, ROE o IDC_max
*/
...
{	
	...
	if(flagROE || flagTemp || flagIdc)
	{
		...
		regAlarm = flagROE << 2 | flagTemp << 1 | flagIdc;
		DPIN->PIO_CODR |= END; 
		apagarEtapaPotencia(); // Se apaga el DC-DC y se deja de alimentar la EP
							   // si se ha superado algun umbral
	}
}
